...
def eulerian_magnification(vid_data, fps, freq_min, freq_max, amplification, pyramid_levels=4, skip_levels_at_top=2):
    vid_pyramid = create_laplacian_video_pyramid(
        vid_data, pyramid_levels=pyramid_levels)
    for i, vid in enumerate(vid_pyramid):
        ...
        bandpassed = temporal_bandpass_filter(
            vid, fps, freq_min=freq_min, freq_max=freq_max, amplification_factor=amplification)
...
def show_frequencies(vid_data, fps, bounds=None):
    ...
    if bounds:
        for x in range(1, vid_data.shape[0] - 1):
            averages.append(
                vid_data[x, bounds[2]:bounds[3], bounds[0]:bounds[1], :].sum())
    else:
        for x in range(1, vid_data.shape[0] - 1):
...
def gaussian_video(video, shrink_multiple):
    ...
    for x in range(0, video.shape[0]):
        ...
        if x == 0:
            vid_data = np.zeros(
                (video.shape[0], gauss_copy.shape[0], gauss_copy.shape[1], 3))
...
def laplacian_video(video, shrink_multiple):
    ...
    for i, frame in enumerate(video):
        ...
        if vid_data is None:
            vid_data = np.zeros(
                (frame_count, laplacian.shape[0], laplacian.shape[1], 3))
